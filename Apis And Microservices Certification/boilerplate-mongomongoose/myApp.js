...
var findOneByFood = function(food, done) {
  ...
  Person.findOne({favoriteFoods:{ $exists: true, $in: food}},function(err,data){
    done(null, data);
  });
...
var findEditThenSave = function(personId, done) {
  ...
  // Person.update({_id:personId},{ $push: {favoriteFoods: foodToAdd } },function(err,data){
  //   done(null, data);
  // });
};
...
var queryChain = function(done) {
  ...
  Person.find({favoriteFoods:{ $exists: true, $in: foodToSearch}}).sort({name:1}).limit(2).select({age:0}).exec(function(err,data){
    done(null, data);
  });
};
